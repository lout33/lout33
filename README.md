# Luis Fernando Yupanqui

**AI Engineer. I ship fast.**

- **632+ GitHub stars** on Claude Life Assistant (top 0.1% of GitHub projects)
- **HN frontpage** — 187 points, 8,300+ sessions, international press (Gigazine Japan)
- **4B+ images/year** — Built real-time CV systems at Deepview (AWS, Kubernetes)
- **Full rewrites in days** — Not months

## Projects

| Project | Results | Links |
|---------|---------|-------|
| **Claude Life Assistant** | 632+ stars, top 0.1% GitHub | [Code](https://github.com/lout33/claude_life_assistant) |
| **So Long Sucker** | HN frontpage (187 pts), 8K+ users, 162 games, 15K AI decisions | [Demo](https://so-long-sucker.vercel.app) · [Code](https://github.com/lout33/so-long-sucker) |
| **Canvas Agent** | LLM + Gemini + Veo 3 video generation | [Demo](https://canvas-agent.vercel.app) · [Code](https://github.com/lout33/canvas-agent) |
| **ESP32 AI Assistant** | Voice AI on $10 hardware, 1.2K YouTube views | [Video](https://youtube.com/watch?v=pYbnEi_xh0M) · [Code](https://github.com/lout33/ai_assistant_esp32) |

## Stack

Python · TypeScript · Node.js · FastAPI · React · AWS · Kubernetes · Terraform · LLMs · Computer Vision

## Links

[LinkedIn](https://linkedin.com/in/luis-yupanqui) · [Portfolio](https://luisfernandoyt.vercel.app) · [YouTube](https://www.youtube.com/@code4fun677) · [Substack](https://substack.com/@fernandoyt)

---

**Looking for AI Engineer roles.**

lyupanqui99@gmail.com
